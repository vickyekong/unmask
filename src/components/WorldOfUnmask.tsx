...
const worldElements = [
  {
    title: "The Archive",
    description:
      "A fictional society of readers, writers, and seekers who believe identity is narrative — and narrative is sacred. Members are called Keepers.",
    icon: "◈",
  },
  {
    title: "The Circle",
  ...
  return (
    <section id="world" className="section-padding relative parchment">
      <div className="mx-auto max-w-6xl">
        <Reveal>
          <p className="museum-label mb-4 text-library-brown/60">III · The World</p>
        </Reveal>

        <Reveal delay={0.1}>
          <h2 className="font-heading text-4xl md:text-5xl lg:text-6xl font-light text-library-brown">
            The World of{" "}
            <span className="italic">Unmask</span>
          </h2>
        </Reveal>
        ...
        <div className="mt-16 grid gap-6 md:grid-cols-2">
          {worldElements.map((element, i) => (
            <Reveal key={element.title} delay={0.2 + i * 0.1}>
              <article className="group flex gap-6 border border-library-brown/10 bg-aged-paper/50 p-8 transition-all hover:border-burnished-gold/30 hover:shadow-lg hover:shadow-library-brown/5">
                <span className="font-heading text-3xl text-burnished-gold/50 transition-colors group-hover:text-burnished-gold">
                  {element.icon}
                </span>
                <div>
        ...
        <Reveal delay={0.6} className="mt-20">
          <div className="relative overflow-hidden border border-library-brown/15 bg-library-brown/5 p-10 md:p-16">
            ...
            <p className="font-heading text-xl md:text-2xl italic text-library-brown/80 leading-relaxed max-w-3xl">
              &ldquo;We founded UNMASK on a single conviction: that the most
